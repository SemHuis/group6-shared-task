...
def evaluate_on_validation(model_path, adapter_path, valid_file, output_file):
    """
    Evalúa el modelo fine-tuned con few-shot en valid_data.json
    """
    # Cargar modelo
    ...
    print(f"\nevaluating {len(valid_data)} samples (few-shot trained model)")
    ...
    # Imprimir resultados
    print("\n" + "=" * 50)
    print("RESULTS (Few-Shot Trained Model)")
    print("=" * 50)
    ...
    # Guardar resultados
    results_dict = {
        'model': 'LLaMAX3-8B-fewshot-trained',
        'method': 'few-shot training + few-shot inference',
        'metrics': metrics,
        'predictions': results,
        'error_analysis': {
...
if __name__ == "__main__":
    ...
    ADAPTER_PATH = "/gaueko1/users/mmartin/shared_task/models/llamax3-8b-fewshot-final"
    VALID_FILE = "/gaueko1/users/mmartin/shared_task/data/valid_data.json"
    OUTPUT_FILE = "/gaueko1/users/mmartin/shared_task/results/evaluation_fewshot_trained.json"

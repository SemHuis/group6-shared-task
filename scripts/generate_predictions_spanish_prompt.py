...
PRED_FILE = PROJECT_ROOT / "results" / "qwen_spanish" / "predictions_fewshot_spanish_prompt.json"  # 输出的预测文件
...
def build_prompt_from_syllogism(syllogism: str) -> str:
    return f"""Analiza el siguiente silogismo y determina si es lógicamente válido o inválido. Considera solo la estructura lógica formal y no te dejes influir por la plausibilidad del contenido.
Ejemplo 1:
Silogismo: Todos los perros son animales. Todos los animales son seres vivos. Por tanto, todos los perros son seres vivos.
Respuesta: válido

Ejemplo 2:
Silogismo: Ningún gato es un perro. Algunos animales son perros. Por tanto, algunos animales son gatos.
Respuesta: inválido

Silogismo: {syllogism}

¿Es este silogismo lógicamente válido? Responde con 'válido' o 'inválido'.
Respuesta:"""


# ====== 4. 准备 ' valid' 和 ' invalid' 的 token id，并用 logits 做二分类 ======
def prepare_label_token_ids(tokenizer):
    valid_ids = tokenizer(" válido", add_special_tokens=False).input_ids
    invalid_ids = tokenizer(" inválido", add_special_tokens=False).input_ids

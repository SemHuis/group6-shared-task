...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Evaluate LLM predictions and calculate bias metrics.")
    
    parser.add_argument(
        '--gt_file',
        type=str,
        default="/home1/s4779428/shared-task/valid_data.json",
        help="Path to the ground truth (validation) JSON file."
    )
    ...
    parser.add_argument(
        ...
        help="Path to the model's predictions JSON file."
    )
    
    parser.add_argument(
        '--model_name',
        type=str,
        default="My Model Results",
        help="A name for the model, used in the final report."
    )
    ...
    try:
        ...
        # Define the models to analyze (using parsed args)
        models_to_analyze = {
            args.model_name: my_predictions
            # You can still manually add more files here for comparison if needed
        }
        
        all_model_results = []

        # Step 1: Analyze models individually

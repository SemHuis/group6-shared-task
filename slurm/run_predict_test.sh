#!/bin/bash
#SBATCH --job-name=predict_test_14b_aug
#SBATCH --output=logs/predict_test_%j.log
#SBATCH --error=logs/predict_test_%j.err
...
echo ">>> [Status] Running TEST inference for model: ${MODEL_NAME}"
...
# Run post-processing to generate submission format
if [ -f "configs/predict/post_process.py" ]; then
    ...
    echo "⚠️ Warning: post_process.py not found"
fi

echo "✅ All TEST prediction tasks completed successfully!"